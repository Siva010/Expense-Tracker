...
const TransactionList = ({ transactions, title = "Transactions", type, onDelete, hideDeleteBtn }) => {
  ...
  return (
    <div className={`card h-full flex flex-col ${
      isDark 
        ? "bg-gray-800" 
        : "bg-white shadow-[0_8px_30px_-4px_rgba(99,102,241,0.25)] hover:shadow-[0_12px_40px_-4px_rgba(99,102,241,0.35)]"
    }`}> 
      <div className="flex-shrink-0 flex items-center justify-between mb-4">
        <h5 className={`text-lg ${isDark ? "text-black" : "text-black"}`}>{title}</h5>
      </div>
